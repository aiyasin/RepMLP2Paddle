...
def model_load_hdf5(model: paddle.nn.Layer, hdf5_path, ignore_keys='stage0.'):
    ...
    for name, param in model.named_parameters():
        ...
        value = paddle.to_tensor(np_value).float()
...
class WarmupCosineAnnealingLR:
    ...
    def get_lr(self):
        if self.last_epoch < self.warmup:
            return [(self.last_epoch / self.warmup * base_lr) for base_lr in
                self.base_lrs]
        else:
            return [(self.eta_min + (base_lr - self.eta_min) * (1 + math.
                cos(math.pi * (self.last_epoch - self.warmup) / (self.
                T_cosine_max - self.warmup))) / 2) for base_lr in self.base_lrs
                ]
